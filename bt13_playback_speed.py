...
class VideoPlayer:
    ...
    def send_seek_event(self):
        ...
        if self.rate > 0:
            seek_event = Gst.Event.new_seek(
                self.rate, Gst.Format.TIME, seek_flags,
    ...
    def handle_keyboard(self, key):
        if key == 'p':
            ...
            step_event = Gst.Event.new_step(Gst.Format.BUFFERS, 1, abs(self.rate), True, False)
    ...
    def run(self):
        logger.info("USAGE: Press a key and hit enter: [P] Play/Pause, [S/s] Speed Up/Down, [D] Reverse, [N] Next Frame, [Q] Quit")
